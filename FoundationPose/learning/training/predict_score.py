...
class ScorePredictor:
  def __init__(self, ckpt="/../../weights/", amp=True):
    self.amp = amp
    self.run_name = "2024-01-11-20-02-45"

    model_name = 'model_best.pth'
    code_dir = os.path.dirname(os.path.realpath(__file__))
    ckpt_dir = os.path.join(code_dir, ckpt, self.run_name, model_name)
    #ckpt_dir = f'{code_dir}/../../weights/{self.run_name}/{model_name}'

    #self.cfg = OmegaConf.load(f'{code_dir}/../../weights/{self.run_name}/config.yml')
    self.cfg = OmegaConf.load(os.path.join(code_dir, ckpt, self.run_name, "config.yml"))
